...
// disassemble the executable section into ARMv7 instructions
void disas_section(Elf32_Shdr *shdr, FILE *fp, char *sname)
{
	if(fp == NULL)
	{
	...
	int num = shdr->sh_size/SIZE_INST;
	int i;
	...
	for(i=0; i<num; i++)
	{
		...
		{
			//printf("0x%08x-> %s\n", i, str.total);
			printf("0x%08x-> 0x%08x  %s\n", shdr->sh_addr+i*4, tmp, str.total);
		}
	}
...
int main(int argc, char **argv)
{
	...
	// if the section is executable, disassemble it!
	for(i=0; i<shnum; i++)
	{
		if(shdr[i].sh_flags&0x4)
			disas_section(&shdr[i], fp, sname);
	}
